...
source_dir="$(cd $PWD/../source; pwd -P)" # NOTE: source_dir is changed to follow this project's structure.
coverage_reports_top_path=$source_dir/test/coverage-reports

# Skip as the CDK project has no unit tests
# run_cdk_project_test $source_dir/backend

# Test the attached Lambda function
run_javascript_test $source_dir/backend/lambda lambda-functions
